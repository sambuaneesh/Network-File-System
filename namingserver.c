...
int main()
{
    ...
    while (1)
    {
        ...
        else if (strcmp("3", opt) == 0)
        {
            ...
            strcpy(temp_option, create_option);
            ...
            if (send(ns_sock, temp_file_path, sizeof(temp_file_path), 0) == -1)
                printf("[-]Send error\n");

           // printf(" ");
            //  Sending option to the SS

            if (send(ns_sock, temp_option, sizeof(temp_option), 0) == -1)
                printf("[-]Send error\n");
